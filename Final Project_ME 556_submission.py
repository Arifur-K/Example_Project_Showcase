...
def load_occupancy_grid(image_path,resize_factor=1.0,threshold=127,invert=False,show_intermediate=False):
    """Load an image and convert it into an occupancy grid: 0 = free; 1 = obstacle
...
def steer(x_from, y_from, x_to, y_to, step_size):
    """Return a new point that is step_size away from (x_from, y_from) in the direction of 
    (x_to, y_to), unless the distance is smaller than step_size, in which case return (x_to, y_to)."""
    dx = x_to - x_from
    dy = y_to - y_from
    dist = math.hypot(dx, dy)
    if dist <= step_size:
...
def get_near_nodes(nodes, x_new, y_new, radius):
    #Return list of indices of nodes that are within 'radius' of (x_new, y_new).
    near_indices = []
    r2 = radius * radius
    for i, node in enumerate(nodes):
        dx = node.x - x_new
...
# RRT* main routine
def rrt_star(occ_grid,start,goal,max_iterations=5000,step_size=10.0,goal_radius=10.0,gamma_rrt=50.0,max_neighbor_radius=30.0,goal_sample_rate=0.05,show_progress=False):
    """
    ...
    Args:
        ...
        goal_sample_rate: with this probability, sample the goal directly (biases tree growth toward the goal)
    ...
    for it in range(1, max_iterations + 1):
        ...
        radius = min(max_neighbor_radius, gamma_rrt*math.sqrt(math.log(n_nodes)/n_nodes))
...
def plot_rrt_star_result(occ_grid, nodes, goal_index, start, goal):
    ...
    Internally: occ_grid[y, x], y=0 at TOP, y increases DOWN.
    Visually: x increases right, y increases UP (0 at bottom, like math coords)."""
    h, w = occ_grid.shape
    ...
    for node in nodes:
        if node.parent != -1:
            ...
            plt.plot([parent.x, node.x],[y_plot(parent.y), y_plot(node.y)],linewidth=0.5,color="orange",alpha=0.5,)
...
def main():
    ...
    # Replace this with the desired map file.
    image_path = "map_example1.png"  # <-- change to map_example2 to see the other map
    occ_grid = load_occupancy_grid(image_path,resize_factor=1.0,threshold=250,invert=False,show_intermediate=True)
    h, w = occ_grid.shape
